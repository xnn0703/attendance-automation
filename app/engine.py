...
import os, re, glob, copy, datetime
import openpyxl
    ...
    def dates_in(txt):
        ...
        for mm in re.finditer(r'(\d{4})[/-](\d{1,2})[/-](\d{1,2})', s(txt)):
            if int(mm.group(1)) == y and int(mm.group(2)) == mo:
                out.append(int(mm.group(3)))
        return out
    # 个人调班：原日期(O)+现调日期(P)两列涉及的当月日期，一律按基准日历"对调"班/休（翻转）。
    # 两列谁=班谁=休不固定，由基准日历决定：基准班→改休、基准休→改班。
    for row in range(3, ws.max_row + 1):
        nm = s(ws.cell(row, 13).value).strip()        # M 姓名
        if not nm:
            continue
        for dd in dates_in(ws.cell(row, 15).value) + dates_in(ws.cell(row, 16).value):  # O + P
...
# ---------- punch parse ----------
def parse_day(txt):
    prev = None; off = 0; first = None; le = None; cnt = 0; wq = False
    for ln in s(txt).split('\n'):
        ...
        if m:
            ...
            if cnt == 0:
                first = mm
            le = eff; prev = mm; cnt += 1
        if WAIQIN in ln:
            wq = True
    return {'cnt': cnt, 'first': first, 'le': le, 'wq': wq}

def ru30(m):
    import math
    return int(math.ceil(m / 30.0) * 30)

def rd30(m):
    import math
    return int(math.floor(m / 30.0) * 30)
...
def read_config(d):
    ...
    for ln in open(f, encoding='utf-8'):
        ...
        if k == 'OT_EXCLUDE':
            ...
            for pair in v.split(','):
                if ':' in pair:
                    g, hm = pair.split(':'); hm = hm.strip()
                    cfg['strict'][g.strip()] = int(hm[:2]) * 60 + int(hm[2:4])
    return cfg
...
def out_name(src_name, frm, to):
    if frm in src_name:
        return src_name.replace(frm, to)
...
def _load_people(d):
    ...
    ws = wb.active
    y, mo = year_month(ws)
    days = (datetime.date(y + (mo // 12), (mo % 12) + 1, 1) - datetime.timedelta(days=1)).day
    roster = load_roster(inp['ros'])
    cal = load_calendar(inp['tiao'], y, mo)
    ppl = []
    for r in range(5, ws.max_row + 1):
...
def build(d, classify=None, config=None):
    ...
    ws = ctx['ws']; days = ctx['days']; NCOL = 37
    base_font = Font(name=FONT_NAME, size=12, color=RED)
    summary = []
    # precompute per person: styles to apply, OT row, quan, then write bottom-up (insert OT rows)
    plan = []; cells_meta = []
    for p in ctx['ppl']:
        win = window(ctx['roster'], p['bare'], ctx['y'], ctx['mo'], days)
        is_excl = p['gh'] in cfg['excl']
        ot_by = {}; ot_tot = 0.0; quan = True; styles = {}
        for dd in range(1, days + 1):
            if dd < win['s'] or dd > win['e']:
                continue
            is_w = is_work(ctx['cal'], p['bare'], dd)
            pi = parse_day(p['dmap'][dd])
            if is_w and pi['cnt'] < 2:                 # 可人工改判的格子（输出行=2*初表行-5）
                cells_meta.append({'key': '%s|%d' % (p['gh'], dd), 'gh': p['gh'], 'name': p['bare'],
                                   'day': dd, 'row': 2 * p['row'] - 5, 'col': dd + 6})
            fill, fred = day_style(pi, p['gh'], dd, is_w, win, cfg, cls)
            if fill or fred:
                styles[dd] = (fill, fred)
            ot = ot_val(pi, is_w, is_excl)
            if ot > 0:
                ot_by[dd] = ot; ot_tot += ot
            if breaks_quan(pi, p['gh'], dd, is_w, cls):
                quan = False
        plan.append({'p': p, 'styles': styles, 'ot_by': ot_by, 'quan': quan})
        summary.append({'gh': p['gh'], 'name': p['bare'], 'ot': ot_tot, 'quan': quan, 'excl': is_excl})
    # apply bottom-up so inserted OT rows don't shift not-yet-processed rows
    for item in sorted(plan, key=lambda x: x['p']['row'], reverse=True):
        p = item['p']; r = p['row']
        # color day cells + name
